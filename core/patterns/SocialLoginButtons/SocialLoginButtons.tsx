...
const buttonVariants = cva(
  [
    'inline-flex items-center justify-center gap-3 rounded-md border',
...
type Provider = keyof typeof providerIcons
...
const SocialLoginButtons = React.forwardRef<
  HTMLDivElement,
...
>(
  (
    ...
    if (isIconsOnly) {
      return (
        <div ref={ref} className={cn('flex justify-center gap-3', className)}>
          {providers.map((provider) => (
            ...
            >
              {loading === provider ? (
                <div className="h-5 w-5 animate-spin rounded-full border-2 border-[var(--border-default)] border-t-primary-500" />
              ) : (
      ...
      >
        {providers.map((provider) => (
          <button
            ...
            disabled={disabled || loading === provider}
            className={cn(
              buttonVariants({ size, fullWidth: !isCompact }),
              isCompact && 'flex-1'
            )}
          >
            {loading === provider ? (
